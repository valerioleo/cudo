...
> Verify contracts on Etherscan when [cudo](../cudo) deploys them.

A cudo verifier is just a deploy-lifecycle hook. On each fresh deploy this plugin submits the contract's standard-json-input to the **Etherscan V2 API** (one endpoint, one key, every supported chain) and polls until the explorer confirms it.

## Install

```bash
pnpm add -D @cudo/etherscan
```
...
import { etherscan } from "@cudo/etherscan";
...
It reads everything it needs from the deployment — address, chainId, ABI, constructor args — plus the compiler metadata cudo captures at deploy time (fully-qualified name, compiler version, standard-json input). Constructor arguments are ABI-encoded automatically. Reused deployments (no transaction) are skipped.
