...
export interface RunGenerateOptions {
  ...
  readonly out: string;
  /** Absolute path to the user's cudo config (the deployers import it). */
  readonly configPath: string;
  /** Which contracts to generate for. Default: all (with bytecode). */
  readonly include?: ReadonlyArray<string> | RegExp;
  /** Runtime package the generated deployers import. Default "cudo". */
...
/** detect → read → filter → generate. The testable core of `cudo generate`. */
