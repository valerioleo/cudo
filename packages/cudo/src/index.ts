/**
 * cudo — viem-first contract deployment.
 *
 * The user-facing flow is: run `cudo generate` → import a generated per-contract
 * deployer → call it with a viem client. So the public surface is:
 *   - `defineConfig`  — author cudo.config.ts
...
// Generated-deployer factory (emitted by `cudo generate`; users call its result)
