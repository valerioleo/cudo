...
const run = () => {
  const outDir = mkdtempSync(join(tmpdir(), "cudo-gen-"));
  generate([counterArtifact], { outDir, configImport: "../cudo.config" });
  return outDir;
};
...
describe("generate (codegen)", () => {
  ...
  it("emits a self-contained artifact module (abi as const, type-only cudo import)", () => {
    const mod = readFileSync(join(run(), "types", "Counter.ts"), "utf8");
    expect(mod).toContain('import type { TypedArtifact } from "cudo"');
  ...
  it("emits a deployer that wires defineDeployer with the config and artifact", () => {
    ...
    expect(mod).toContain('import { defineDeployer } from "cudo"');
    expect(mod).toContain('import config from "../cudo.config"');
