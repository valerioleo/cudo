...
> Notify a Slack channel when [cudo](../cudo) deploys a contract.

A cudo plugin is just a deploy-lifecycle hook — the same shape a verifier uses. This one posts to a Slack Incoming Webhook each time a contract is freshly deployed.

## Install

```bash
pnpm add -D @cudo/slack
```
...
slack({
  ...
  username: "cudo-bot", // optional bot name
