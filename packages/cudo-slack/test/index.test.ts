...
import type { DeployedContext, DeploymentRecord, PluginDeps } from "cudo/plugin";
...
describe("slack plugin", () => {
  ...
  it("includes the bot username when configured", async () => {
    const { deps, fetch } = makeDeps(new Response("ok", { status: 200 }));
    await run(slack({ webhook: "https://hooks.slack.test/abc", username: "cudo-bot" }), makeCtx(), deps);
    expect(postedBody(fetch).username).toBe("cudo-bot");
  });
});
